...
面向团队与企业部署，LazyMind 把数据隔离、权限边界与行为可追溯放在同一控制平面下处理：每一项操作都明确归属租户，每一类能力都经过授权后才能被调用，每一笔关键动作都可以被回放与审计，从而让多租户环境在保障协作效率的同时，兼顾合规、安全与可问责。

多租户数据隔离贯穿知识库、对话、Artifact、Skill 与 Workflow 的执行上下文：知识库按租户维护独立的文档、向量与解析产物，对话与 Artifact 绑定租户命名空间并保留来源回溯；Skill 与 Workflow 在执行时携带租户上下文与独立的运行凭证，跨租户访问由控制平面统一拦截；资源配额、API Key、模型凭证与外部服务凭据都按租户分别管理，避免共享账号与串用带来的泄漏面。

RBAC 细粒度权限管控覆盖角色定义、权限点、能力授权与外部 Agent 授权边界：内置管理员、成员、只读与审计等基础角色，并允许企业按业务定义自定义角色；权限点细化到知识库读写、Artifact 审阅、Skill 安装、Workflow 发布、模型与工具调用等具体动作，未被显式授予的权限默认拒绝；外部 Agent 的能力调用同样纳入授权范围，能力类型、可用工具、数据来源与执行上限都按授权清单生效，越权请求会被拦截并写入审计。

操作审计日志覆盖关键操作、权限变更、外部 Agent 调用以及知识与 Workflow 执行：每一次数据访问、权限变更、模型调用、外部 Agent 任务与 Workflow 步骤都会记录操作者、租户、时间、来源与结果，并保留足够的上下文用于追溯与复盘；审计记录支持按租户、操作者、时间窗与动作类型检索，可导出至 SIEM 或合规留痕系统，满足内审与外部合规检查的需要。

![企业级多租户隔离与安全权限架构示意图](assets/2e/2e56be0aad5e5780ea5137608199eb1c65c36ea7737f6525f8788d53f4abb754.jpg)
