...
面向团队和企业部署场景，LazyMind 在多租户数据隔离、RBAC 细粒度权限管控和操作审计日志三个层面提供企业级安全合规能力，确保不同租户在同一实例中的语义和资源严格隔离。

**多租户数据隔离**：以租户（tenant）为边界划分命名空间，知识库、Skill、Workflow、Artifact 与任务会话在存储、检索和向量索引层面相互隔离；跨租户访问默认拒绝，仅在显式授权时放行，避免数据在不同组织之间泄漏。

**RBAC 细粒度权限管控**：基于角色（Role）的访问控制（RBAC）按用户、角色与资源三维度配置权限，覆盖知识库读写、Skill 与 Workflow 的执行、Artifact 的查看/编辑/导出、任务中心与能力中心的操作；权限可下发至租户、组织和项目层级，并与 Kong、JWT、Core ACL 等基础设施联动，支持最小权限原则。

**操作审计日志**：完整记录用户登录、知识访问、Skill 与 Workflow 执行、Artifact 修改、权限变更和后台任务的关键操作，审计字段包含操作者、时间、资源、操作类型与结果，并支持按租户、用户和时间范围检索导出，便于合规审查与问题回溯。

![企业级多租户隔离与安全权限架构示意图](assets/21/21e1cf382707726d8f02b98a90dc108ba7c3db962f06d98141aca5b0214a74d9.jpg)
