...
type WorkflowStepParams struct {
	...
	StartupInputs       map[string]any      `json:"startup_inputs,omitempty"`
...
func (p WorkflowStepParams) asMap() map[string]any {
	...
	if len(p.StartupInputs) > 0 {
		m["startup_inputs"] = p.StartupInputs
	}
	if len(p.Filters) > 0 {
		m["filters"] = p.Filters
	}
...
func launchWorkflowAttempt(
	...
	if len(params.StartupInputs) > 0 {
		rawParamsMap["startup_inputs"] = params.StartupInputs
	}
