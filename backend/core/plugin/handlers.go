...
func toSessionDTO(s *orm.PluginSession) sessionDTO {
	return sessionDTO{
		...
		PluginID:         s.PluginID,
		PinnedRevisionID: s.PluginRevisionID,
		PinnedRevisionNo: s.PluginRevisionNo,
		Status:           s.Status,
...
func enrichSessionHeadRevisionNos(ctx context.Context, db *gorm.DB, sessions []sessionDTO) []sessionDTO {
	...
	var revisions []orm.PluginRevision
	...
	for _, revision := range revisions {
		resourceIDByRevision[revision.ID] = revision.PluginResourceID
		resourceIDs = append(resourceIDs, revision.PluginResourceID)
	}
	if len(resourceIDs) == 0 {
		return sessions
	}
	var resources []orm.PluginResource
